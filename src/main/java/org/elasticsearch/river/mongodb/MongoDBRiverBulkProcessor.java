...
public class MongoDBRiverBulkProcessor {
    ...
    private final Listener listener = new Listener() {
        ...
        public void beforeBulk(long executionId, BulkRequest request) {
            ...
            log.trace("beforeBulk - new bulk [{}] of items [{}]", executionId, request.numberOfActions());
            if (flushBulkProcessor.get()) {
                log.trace("About to flush bulk request index[{}] - type[{}]", index, type);
                ...
                try {
                    ...
                    log.error("Drop collection operation failed", t);
        ...
        private int findLastDropCollection(List<DocWriteRequest> request) {
            int index = 0;
            for (int i = 0; i < request.size(); i++) {
                DocWriteRequest action = request.get(i);
        ...
        public void afterBulk(long executionId, BulkRequest request, Throwable failure) {
            if (failure.getClass().equals(ActionRequestValidationException.class)) {
                if (log.isTraceEnabled()) {
                    log.trace("Ignore ActionRequestValidationException : {}", failure);
                }
            } else {
                log.error("afterBulk - Bulk request failed: {} - {} - {}", executionId, request, failure);
                MongoDBRiverHelper.setRiverStatus(client, definition.getRiverName(), Status.IMPORT_FAILED);
                request.requests().clear();
                bulkProcessor.close();
                river.close();
        ...
        public void afterBulk(long executionId, BulkRequest request, BulkResponse response) {
            if (response.hasFailures()) {
                log.error("Bulk processor failed. {}", response.buildFailureMessage());
                MongoDBRiverHelper.setRiverStatus(client, definition.getRiverName(), Status.IMPORT_FAILED);
                request.requests().clear();
                bulkProcessor.close();
                river.close();
                ...
                logStatistics(response.getIngestTookInMillis());
                deletedDocuments.set(0);
                updatedDocuments.set(0);
                insertedDocuments.set(0);
                if (log.isTraceEnabled()) {
                    log.trace("afterBulk - bulk [{}] success [{} items] [{} ms] total [{}]", executionId, response.getItems().length,
                            response.getIngestTookInMillis(), documentCount.get());
                }
            }
        }
    };
    ...
    public void deleteBulkRequest(String id, String routing, String parent) {
        log.trace("deleteBulkRequest - id: {} - index: {} - type: {} - routing: {} - parent: {}", id, index, type, routing, parent);
        ...
        while (!isBulkProcessorAvailable()) {
            try {
                if (log.isDebugEnabled()) {
                    log.debug("Waiting for bulk queue to empty...");
                }
                Thread.sleep(2000);
            } catch (InterruptedException e) {
                log.warn("checkIndexStatistics interrupted", e);
            }
        }
    }
        ...
        for (NodeStats nodeStats : response.getNodes()) {
            ...
            while (iterator.hasNext()) {
                ...
                if ("bulk".equals(stats.getName())) {
                    ...
                    log.trace("bulkQueueSize [{}] - queue [{}] - availability [{}]", bulkQueueSize, queue, 1 - (queue / bulkQueueSize));
    ...
    private void dropRecreateMapping() throws IOException, InterruptedException {
        try {
            semaphore.acquire();
            log.trace("dropRecreateMapping index[{}] - type[{}]", index, type);
            client.admin().indices().prepareRefresh(index).get();
            ImmutableOpenMap<String, MappingMetaData> mappings = client.admin().cluster().prepareState().get().getState().getMetaData().index(index).getMappings();
            log.trace("mappings contains type {}: {}", type, mappings.containsKey(type));
            if (mappings.containsKey(type)) {
                ...
                DeleteIndexTemplateRequestBuilder deleteIndexTemplateRequestBuilder = indicesAdminClient.prepareDeleteTemplate(index).setName(type);
                DeleteIndexTemplateResponse deleteIndexTemplateResponse = indicesAdminClient.deleteTemplate(deleteIndexTemplateRequestBuilder.request()).actionGet();
                if (deleteIndexTemplateResponse.isAcknowledged()) {
                    ...
                    if (!pmr.isAcknowledged()) {
                        log.error("Failed to put mapping {} / {} / {}.", index, type, mapping.source());
                    } else {
                        log.info("Delete and recreate for index / type [{}] [{}] successfully executed.", index, type);
                    }
                } else {
                    log.warn("Delete type[{}] on index[{}] return aknowledge false", type, index);
                }
            } else {
                log.info("type[{}] does not exist in index[{}]. No need to remove mapping.", index, type);
            }
        } finally {
            semaphore.release();
        }
    ...
    private void logStatistics(long duration) {
        if (definition.isStoreStatistics()) {
            ...
            log.trace("Indexed {} documents: {} insertions, {} updates, {} deletions", totalDocuments, insertedDocuments.get(),
